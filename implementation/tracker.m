...
im_sz = size(im);
config_detc=det_config(target_sz, im_sz);
svm_struct=[];
...
while true
    ...
    if seq.frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            [trans_row, trans_col, scale_ind, max_score] = optimize_scores(scores_fs, params.newton_iterations);
            if seq.frame == 2
                response_ref = max_score;
            end
        ...
        if mod(seq.frame, det_interval) == 0
            bbs = edgeBoxes(im, model, opts_det);
            if ~isempty(bbs)
                scores = [];
                for bb_idx=1:min(1000, length(bbs))
                    bb = bbs(bb_idx, 1:4);
                    bb_im = im(bb(2)+1:bb(2)+bb(4), bb(1)+1:bb(1)+bb(3), :);
                    bb_im = imresize(bb_im, config_detc.t_sz);
                    bb_feat = double(fhog(single(bb_im) / 255, 4, 9));
                    bb_feat(:,:,end) = [];
                    scores(bb_idx) = svm_struct.w' * bb_feat(:) + svm_struct.b;
                end
                [scores_sort, scores_idx] = sort(scores, 'descend');
                bb_det = bbs(scores_idx(1), 1:4);
                bb_track = [pos(2)-target_sz(2)/2, pos(1)-target_sz(1)/2, target_sz(2), target_sz(1)];
                det_track_r = overlap_ratio(bb_det,bb_track);
                if det_track_r == 0
                    pos_left = [bb_det(2)+bb_det(4)/2, bb_det(1)+bb_det(3)/2];
                    sample_pos = round(pos_left);
                    det_sample_pos = sample_pos;
                    sample_scale = currentScaleFactor*scaleFactors;     
                    xt = extract_features(im, sample_pos, sample_scale, features, global_fparams, feature_extract_info);
                    % Project sample
                    ...
                    % Optimize the continuous score function with Newton's method.
                    [trans_row, trans_col, scale_ind, max_score_det] = optimize_scores(scores_fs, params.newton_iterations);        

                    if max_score_det > max_score  && max_score_det > (response_ref * th1)
                        translation_vec = [trans_row, trans_col] .* (img_support_sz./output_sz) * currentScaleFactor * scaleFactors(scale_ind);

                        max_score = max_score_det;
                        bb_det(1) = bb_det(1) + translation_vec(2);
                        bb_det(2) = bb_det(2) + translation_vec(1);
                        pred_Loc = bb_det;
                        pos = [pred_Loc(2)+pred_Loc(4)/2, pred_Loc(1)+pred_Loc(3)/2];
                        currentScaleFactor = sqrt(pred_Loc(4) * pred_Loc(3) / (base_target_sz(1) * base_target_sz(2)));
                    end
                end
            end
        end
    ...
    if seq.frame == 1
        ...
        % sampling policy
        opts.trans_f = 0.3;     % translation std: mean(width,height)*trans_f/2
        opts.scale_f = 5;       % scaling std: scale_factor^(scale_f/2)

        targetLoc = [pos(2)-target_sz(2)/2, pos(1)-target_sz(1)/2, target_sz(2), target_sz(1)];
        ...
        feat = [];
        labels = [];
        pos_examples(:,1) = min(opts.imgSize(2)-pos_examples(:,3), max(1, pos_examples(:,1)));
        pos_examples(:,2) = min(opts.imgSize(1)-pos_examples(:,4), max(1, pos_examples(:,2)));
        
        for bb_idx=1:length(pos_examples)                    
            bb = pos_examples(bb_idx, 1:4);
            bb_im = im(bb(2)+1:bb(2)+bb(4), bb(1)+1:bb(1)+bb(3), :);
            bb_im = imresize(bb_im, config_detc.t_sz);
            bb_feat = double(fhog(single(bb_im) / 255, 4, 9));
            bb_feat(:,:,end) = [];
        ...
        neg_examples(:,1) = min(opts.imgSize(2)-neg_examples(:,3), max(1, neg_examples(:,1)));
        neg_examples(:,2) = min(opts.imgSize(1)-neg_examples(:,4), max(1, neg_examples(:,2)));
        for bb_idx=1:length(neg_examples)
            bb = neg_examples(bb_idx, 1:4);
            bb_im = im(bb(2)+1:bb(2)+bb(4), bb(1)+1:bb(1)+bb(3), :);
            bb_im = imresize(bb_im, config_detc.t_sz);
            bb_feat = double(fhog(single(bb_im) / 255, 4, 9));
            feat(:, bb_idx+length(pos_examples)) = bb_feat(:);
            labels(bb_idx+length(pos_examples)) = -1;
        end
    ...
    elseif params.learning_rate > 0 && max_score > (response_ref * th1)
    ...
    tracking_result.target_size = double(pred_sz);
    tracking_result.center_pos = double(pred_pos);
...
%         rect_position_vis = [pos([2,1]) - (target_sz([2,1]) - 1)/2, target_sz([2,1])];
        rect_position_vis = [pred_pos([2,1]) - (pred_sz([2,1]) - 1)/2, pred_sz([2,1])];
...
%             alpha(resp_handle, 0.5);
            ...
            text(10, 40, num2str(max_score/response_ref), 'color', [0 1 1], 'FontSize',20);
            hold off;
...
% disp(['min_score: ' num2str(mean(mink(scores, 1000)))])
